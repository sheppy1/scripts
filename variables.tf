variable "backup" {
  type = object({
    ...
    interval_in_hours    = optional(number)
    storage_redundancy   = optional(string)
  })

  validation {
    condition = (
      var.backup.type == "Continuous" || var.backup.type == "Periodic"
    )
  ...
  validation {
    condition     = (
      var.backup.interval_in_hours == null || var.backup.interval_in_hours >= 8 && var.backup.interval_in_hours <= 720
    )
    error_message = "Invalid value for backup.interval_in_hours. Must be between 8 and 720."
  }
